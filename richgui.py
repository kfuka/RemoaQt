...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def open_folder(self):
        if self.id:
            subprocess.run('explorer {}'.format(
                data_base_folder + str(self.id)), cwd=os.getcwd())
        else:
            subprocess.run('explorer {}'.format(data_base_folder),
                           cwd=os.getcwd())
    ...
    def calc(self):
        ...
        for i in range(len(self.rois1)):
            trackroi = [int(self.roi_corrections1[0][i][0] - roi_size / 2),
                        int(self.roi_corrections1[0][i][1] - roi_size / 2),
                        roi_size, roi_size]
        ...
        for i in range(len(self.rois2)):
            trackroi = [int(self.roi_corrections2[0][i][0] - roi_size / 2),
                        int(self.roi_corrections2[0][i][1] - roi_size / 2),
                        roi_size, roi_size]
    ...
    def generate_table(self):
        self.max_x1, self.max_x2, self.max_y1, self.max_y2, self.max_3d =\
            self.get_maxshift_from_marker_chase()
        data = []
        for i in range(len(self.max_x1)):
            data.append([str(round(self.max_y1[i], 2)),
                         str(round(self.max_x1[i], 2)),
                         str(round(self.max_y2[i], 2)),
                         str(round(self.max_x2[i], 2)),
                         str(round(self.max_3d[i], 2))])
    ...
    def get_maxshift_from_marker_chase(self):
        ...
        for j, i in enumerate(self.marker_chase1):
            ...
            max_x1.append((np.max(x_shifts) - np.min(x_shifts))
                          * self.pixel_spacing1[0])
            max_y1.append((np.max(y_shifts) - np.min(y_shifts))
                          * self.pixel_spacing1[1])
            max_x2.append((np.max(x2_shifts) - np.min(x2_shifts))
                          * self.pixel_spacing1[0])
            max_y2.append((np.max(y2_shifts) - np.min(y2_shifts))
                          * self.pixel_spacing1[1])

        LR_from_center = []
        AP_from_center = []
        for x1 in self.marker_chase1:
            LR_from_center.append((756.0 - np.average(x1[0]))
                                  * self.pixel_spacing1[0])
        for x2 in self.marker_chase2:
            AP_from_center.append((np.average(x2[0]) - 756.0)
                                  * self.pixel_spacing2[0])
        ...
        for n, the_x1 in enumerate(max_x1):
            ...
            c_max_3d.append(
                np.sqrt((self.LRAP_correction(the_x1,
                         AP_from_center[n])) ** 2
                        + (self.LRAP_correction(max_x2[n],
                           LR_from_center[n])) ** 2
                        + (max([self.LRAP_correction(max_y1[n],
                                                     AP_from_center[n]),
                                self.LRAP_correction(max_y2[n],
                                LR_from_center[n])])) ** 2))
    ...
    def mouse_click1(self, event):
        ...
        if current_slice == 0 and not self.iscalculated:
            ...
            roi = pg.RectROI(np.array([event.pos().x(), event.pos().y()]) - 15,
                             ...
                             pen=pg.mkPen(
                                color_waves[
                                    len(self.roi_corrections1[current_slice])
                                    - 1]),
            ...
            if self.mwave1 != []:
                for i in range(len(self.mwave1)):
                    self.graphicsView_3.plot(
                        self.wave_time1, self.mwave1[i],
                        pen=pg.mkPen(color=color_waves[i]),
    ...
    def mouse_click2(self, event):
        ...
        if current_slice == 0 and not self.iscalculated:
            ...
            self.roi_corrections2[current_slice][roi_num] \
    ...
    def roiMove2(self):
        ...
        current_slice = self.horizontalSlider.value() + 1
        for n, roi in enumerate(self.rois2):
            self.roi_corrections2[current_slice][n] \
                = [roi.pos().x(), roi.pos().y()]

        self.graphicsView_4.getPlotItem().clearPlots()
        self.mwave2, maxx, maxy, self.marker_chase2 =\
            self.update_mwave(self.rois2,
                              self.mwave2,
                              self.roi_corrections2, self.pixel_spacing2)
        self.plot_wave2()
        if self.mwave2 != []:
    ...
    def init_graphics(self):
        ...
        self.pixel_spacing2 = np.array([float(self.dicom2.PixelSpacing[0]),
                                        float(self.dicom2.PixelSpacing[1])]) \
            * spacing_correction_factor
    ...
    def plot_wave(self):
        ...
        self.graphicsView_3.plot(self.wave_time1,
                                 self.wave1,
        ...
        self.graphicsView_3.setXRange(np.min(self.wave_time1),
                                      np.max(self.wave_time1),
                                      padding=0.05)
        ...
        self.graphicsView_4.setXRange(np.min(self.wave_time2),
                                      np.max(self.wave_time2), padding=0.05)
        self.graphicsView_4.setYRange(0, 100, padding=0.05)
        self.line1 = pg.InfiniteLine(pos=0, angle=90,
                                     pen=pg.mkPen(color="w", width=0.5))
        self.line2 = pg.InfiniteLine(pos=0, angle=90,
                                     pen=pg.mkPen(color="w", width=0.5))
    ...
    def save_items(self):
        ...
        self.graphicsView.grab().save(directory_path
                                      + "/" + str(self.SOPUID[0]) + "AP.png")
        self.graphicsView_2.grab().save(directory_path
                                        + "/" + str(self.SOPUID[0]) + "LR.png")
    ...
    def create_report(self):
        ...
        tkroot.withdraw()
        if self.id:
            patient_save_folder = filedialog.askdirectory(
                initialdir=data_base_folder + str(self.id))
        else:
            patient_save_folder = filedialog.askdirectory(
                initialdir="./data_base/")
        tkroot.destroy()
        _ = build_report(patient_save_folder)
